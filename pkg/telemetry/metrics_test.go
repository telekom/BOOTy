...
func TestMetricSnapshot_JSON(t *testing.T) {
	...
	if decoded.DiskCount != 2 {
		t.Errorf("diskCount = %d", decoded.DiskCount)
	}
}
...
func TestStepTracker_StartEnd(t *testing.T) {
	...
	if m.StepDuration.Count() != 1 {
		t.Errorf("metric count = %d", m.StepDuration.Count())
	}
}
...
func TestStepStatusConstants(t *testing.T) {
	if string(StatusPending) != "pending" {
		t.Error("StatusPending")
	}
	if string(StatusRunning) != "running" {
		t.Error("StatusRunning")
	}
	if string(StatusDone) != "done" {
		t.Error("StatusDone")
	}
	if string(StatusFailed) != "failed" {
		t.Error("StatusFailed")
	}
	if string(StatusSkipped) != "skipped" {
		t.Error("StatusSkipped")
	}
}
