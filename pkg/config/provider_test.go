...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "network persistence rejects rhel before provisioning", cfg: Config{PersistNetwork: true, OSFamily: "rhel", Network: NetworkConfig{Static: StaticConfig{Iface: "ens3", IP: "10.1.0.5/24"}}}, wantErr: "osFamily rhel target network persistence is blocked until native bootloader, BLS, and SELinux first-boot support are implemented"},
		{name: "network persistence rejects rhel bond before provisioning", cfg: Config{PersistNetwork: true, OSFamily: "rhel", Network: NetworkConfig{Static: StaticConfig{IP: "10.1.0.5/24"}, Bond: BondConfig{Interfaces: "eth0,eth1"}}}, wantErr: "osFamily rhel target network persistence is blocked until native bootloader, BLS, and SELinux first-boot support are implemented"},
