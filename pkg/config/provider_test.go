...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "network persistence rejects rhel bond before provisioning", cfg: Config{PersistNetwork: true, OSFamily: "rhel", Network: NetworkConfig{Static: StaticConfig{IP: "10.1.0.5/24"}, Bond: BondConfig{Interfaces: "eth0,eth1"}}}, wantErr: "osFamily rhel target network persistence does not yet support network.bond.interfaces or network.vlan.config"},
		{name: "network persistence rejects flatcar vlan before provisioning", cfg: Config{PersistNetwork: true, OSFamily: "flatcar", Network: NetworkConfig{VLAN: VLANConfig{Config: "100:eth0:10.100.0.5/24"}}}, wantErr: "osFamily flatcar target network persistence does not yet support network.bond.interfaces or network.vlan.config"},
