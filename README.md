...
- **Declarative partition layouts** — GPT partition + optional LVM layout via `PARTITION_LAYOUT` JSON (leave `IMAGE` unset in layout mode)
...
```bash
export IMAGE="http://images.local/ubuntu-22.04.img.gz"  # Leave unset when PARTITION_LAYOUT is used
...
# Optional declarative layout (requires IMAGE to be unset for now)
...
| `PARTITION_LAYOUT` | — | Optional JSON layout (single-quoted in shell) for GPT partitions and LVM; currently requires `IMAGE` to be unset |
