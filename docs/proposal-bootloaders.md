...
| GRUB path differences (grub vs grub2) | Detection helpers recognize GRUB2 installs, but active provisioning still uses `update-grub` and `/boot/grub/grub.cfg`; add `/boot/grub2` and `grub2-mkconfig` support before claiming RHEL/SUSE GRUB2 handoff |
